...
<template lang="html">
   <div class="">
     <div v-if="!newview">
       ...
       <p class="leyenda">HISTORICO RECEPCION BINS</p>
        <hr>
     </div>
    <!--Panel de creacion de ordenes -->
    <recepbins v-if="newview" :order="item" v-on:getclose="closecreatesupplie()"> </recepbins>
              ...
              <div class="panel panel-default pnl">
                  <div class="panel-heading pnl-heading-fix-t">
                      <div class="row rowheader">
                        ...
                        <div class="col-lg-1 col-md-1 col-sm-2  col-xs-6">Folio</div>
                        <div class="col-lg-2  col-md-2 hidden-sm   col-xs-6">Cliente</div>
                        <div class="col-lg-2 col-md-2 col-sm-2  col-xs-6">Dueño</div>
                        <div class="col-lg-1 col-md-1 col-sm-1  col-xs-6">Cantidad</div>
                        <div class="col-lg-2 col-md-2 col-sm-3 col-xs-6">Estado </div>
                        <div class="col-lg-1 col-md-1 col-sm-2 col-xs-6"></div>
                     </div>
                  </div>
                <div class="panel-body pnl-body-fix-t">
                  <div class="row rowset" v-for="entity in list" :key="entity.id" :class="{recep1: entity.status_id === 1}">
                    ...
                    <div class="col-lg-1 col-md-1 col-sm-2  col-xs-6">{{entity.folio}}</div>
                    <div class="col-lg-2  col-md-2 hidden-sm  col-xs-6">{{getdescrip(entity.client_id, 'clients')}}</div>
                    <div class="col-lg-2 col-md-2 col-sm-2  col-xs-6">{{getdescrip(entity.owner_id, 'clients')}}</div>
                    <div class="col-lg-1 col-md-1 col-sm-1 col-xs-6">{{entity.cant}}</div>
                    <div class="col-lg-2 col-md-2 col-sm-3 col-xs-6">{{getdescrip(entity.status_id, 'status')}}</div>
                    <div class="col-lg-1 col-md-1 col-sm-1 col-xs-6">
                      <button  v-if="entity.status_id !== 5"  @click="info(entity)" class="btn btn-success btn-xs">Informe</button>
                      <button  v-if="entity.status_id !== 5 && entity.scan !== null"  @click="showimg(entity.scan)" class="btn btn-primary btn-xs"><i :class="getclass(entity.scan)"></i></button>
                      <div  v-if="entity.status_id == 5" class="dropdown ">
                        ...
                        <ul class="dropdown-menu dropdown-fix" aria-labelledby="dropdownMenu1">
                          ...
                          <li v-if="entity.scan !== null"><a  @click="showimg(entity.scan)">Guia</a></li>
...
import recepbins from '@/components/recepbins'
export default {
  name: 'emptybins',
  data () {
    return {
  ...
  components: {
    datepicker, paginator, spinner, deletes, vselect, find, recepbins
  },
  mounted () {
    this.getlist()
  },
  methods: {
    ...
    info (aitem) {
      ...
      window.$('#printframe').html('<iframe src="' + urlapi + '/binsemptys/info/' + aitem.id + '"height="100%" width="100%" frameborder="0"></iframe>')
