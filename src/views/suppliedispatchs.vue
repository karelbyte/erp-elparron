<template lang="html">
   <div class="">
     <div v-if="!newview">
       <p class="text-justify"><samp class="txtblack">AYUDA:</samp>Modulo despachar de insumos, utilize el boton "Nuevo".<br>
       <p class="leyenda">HISTORICO DESPACHO DE INSUMOS</p>
        <hr>
     </div>
    <!--Panel de creacion de ordenes -->
    <supdispatchs v-if="newview" :order="item" v-on:getclose="closecreatesupplie()"> </supdispatchs>
         ...
         <div class="col-lg-12">
            <div class="panel panel-default pnl">
              ...
              <div class="panel-body pnl-body-fix-t">
                <div class="row rowset" v-for="entity in list" :key="entity.id" :class="{recep1: entity.status === 4}">
                  <div class="col-lg-1 col-md-2 col-sm-2 hidden-xs">{{entity.datefix}}</div>
                  <div class="col-lg-1 col-md-2 col-sm-2 col-xs-6">{{getdescrip(entity.type_id, 'types')}}</div>
...
import supdispatchs from '@/components/supdispatchs'
export default {
  name: 'suppliedispatchs',
  data () {
    return {
      scan: null,
      entity: '',
  ...
  components: {
    datepicker, paginator, spinner, deletes, vselect, find, supdispatchs
  },
  mounted () {
    this.getlist()
  },
  methods: {
    ...
    news () {
      this.item.datedispatch = new Date()
      this.item.note = '-'
      this.item.act = 'post'
      this.newview = true
    },
    ...
    info (aitem) {
      ...
      window.$('#printframe').html('<iframe src="' + urlapi + '/suppliesdispatchs/info/' + aitem.id + '"height="100%" width="100%" frameborder="0"></iframe>')
      window.$('#view_doc .modal-body').height(450)
      window.$('#view_doc .modal-dialog').width(620)
      window.$('#view_doc').modal('show')
    },
    details () {
      window.$('#printframed').html('<iframe src="' + urlapi + '/suppliesdispatchs/details " height="100%" width="100%" frameborder="0"></iframe>')
