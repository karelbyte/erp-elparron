<template lang="html">
 <div class="">
   <div v-if="newview">
     <p>
       <samp class="txtblack">Frecuencia:</samp>Toda vez que se detecte un producto No Conforme.<br>
       <samp class="txtblack">Procedimiento:</samp>Debe anotarse cantidad de producto y el problema que dio origen a la no conformidad.<br>
      <samp class="txtblack">Acciones Correctivas:</samp>Ante cualquier duda o problema del producto consultar y/o avisar al jefe de Planta o encargado de Aseguramiento de Calidad.<br>
       <p class="text-justify"><samp class="txtblack">AYUDA:</samp> De click <strong >Nueva</strong> para generar datos.<br>
     </p><hr>
     <p class="leyenda"> HISTORICO DE NO CONFORMES ORDEN DE ENVASADO</p>
   ...
   <div v-if="!newview">
      <div class="panel panel-default">
        <div class="panel-heading pnl-heading-fix-t">
           Generador de datos de producto no conforme
        </div>
              ...
              <div class="row">
                ...
                <div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control input-sm" v-model="item.cantbox" >
                  </div>
                </div>
                 <div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
                   <div class="form-group">
                     <label>Problema</label>
                     <select class="form-control input-sm" v-model="item.problem_id">
                       <option v-for="pro in problems" :key="pro.id" :value="pro.id">{{pro.name}}</option>
                     </select>
                   </div>
                 </div>
                <div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
                  <div class="form-group">
                    ...
                    <select class="form-control input-sm" v-model="item.destination_id">
                      <option v-for="des in destinations" :key="des.id" :value="des.id">{{des.name}}</option>
                    </select>
                  </div>
                </div>
                <div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
                  <div class="form-group">
                    <label>Liverado por cliente</label>
                    <select class="form-control input-sm" name="" v-model="item.freefor_id">
                ...
                <div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
                  <div class="form-group">
                    ...
                    <input class="form-control input-sm" v-model="item.note">
                  </div>
                </div>
               </div>
        </div>
         ...
         <div class="panel panel-default pnl">
           <div class="panel-heading pnl-heading-fix-t">
             <div class="row rowheader">
               ...
               <div class="col-lg-2 col-md-2 col-sm-2 hidden-xs">Cant. Cajas</div>
               <div class="col-lg-2 col-md-2 col-sm-2 hidden-xs">Problema</div>
               <div class="col-lg-2 col-md-2 col-sm-2 hidden-xs">Destino</div>
           ...
           <div class="panel-body pnl-body-fix-t">
             <div  class="row rowset" v-for="entity in list" :key="entity.id" >
               ...
               <div class="col-lg-2 col-md-2 col-sm-2 hidden-xs">{{desc(entity.problem_id, 'problems')}} </div>
               <div class="col-lg-2 col-md-2 hidden-sm hidden-xs">{{desc(entity.destination_id, 'destinations')}}</div>
...
export default {
  ...
  methods: {
    getboss () {
      this.axios.get(urlapi + '/packings/getboss/' + this.pack.id + '/8').then(res => {
    ...
    news () {
      ...
      this.item.cantbox = 0
      this.item.note = ''
      this.item.freefor_id = 0
      this.item.aprove = 0
      this.item.problem_id = 0
      this.item.destination_id = 0
