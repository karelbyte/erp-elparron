<template lang="html">
 <div class="" style="margin-bottom:10px">
   <p class="text-justify"><samp class="txtblack">AYUDA:</samp> Modulo de actualizar bins de la orden de calibrado.<br>
                  ...
                  <div class="form-group input-group-sm">
                    <label for="">Productor</label>
                     <select class="form-control" name="" v-model="item.producer_id" @change="gettally()">
                         <option v-for="prod in producers" :value="prod.id" :key="prod.id"> {{prod.name}}</option>
                     </select>
                  </div>
                </div>
                <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
...
export default {
  ...
  data () {
    return {
      ...
      item: {
        ...
        recalibrate: this.order.recalibrate,
        producer_id: 0,
        owner: 0,
        tally: 0,
        caliber_id: 0,
  ...
  methods: {
    ...
    getproducers () {
      this.axios.get(urlapi + '/calibrated/producers/' + this.order.id + '/' + true).then(response => {
    ...
    save () {
      if (this.selected.length > 0) {
        ...
        this.axios({ // guardar orden
          method: this.act,
          url: urlapi + '/calibrated/dropbins',
          data: { data: this.selected, order: {id: this.order.id, folio: this.order.folio, calibrate: this.filters.recalibrate} }
        }).then(response => {
    ...
    getlist (pFil, pOrder, pPager) {
      ...
      this.filters.recalibrate = this.order.recalibrate === 1
